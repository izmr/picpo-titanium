...
@implementation Sample20120419Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_sample20120419);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any Sample20120419 patches in JS before a context is loaded... nice for 
...
{
	// create Sample20120419 global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_sample20120419 = [[Sample20120419Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_sample20120419];
	
	NSString *_sample20120419NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _sample20120419NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_sample20120419 valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.sample20120419.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
